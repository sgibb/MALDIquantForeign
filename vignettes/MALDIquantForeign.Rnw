...
spectra[[1]]
@
Remote files are supported as well. Please note that the data are
filtered and have irregulare intervals between the \textit{m/z} values. That's
why \Rfunction{import} assumes centroided data (peaks). To force
\Rfunction{import} to produce \Robject{MassSpectrum} objects we set the argument
\Robject{centroided=FALSE}.\\
Data are taken from \citet{Tan2006}.
...
\bibliography{MALDIquantForeign}
